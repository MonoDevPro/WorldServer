...
public class IntentsDequeueSystem : BaseSystem<World, float>
{
    ...
    private readonly IntentEnqueueSystem _enqueuer;
    ...
    public IntentsDequeueSystem(ILogger<IntentsDequeueSystem> logger, 
        World world, IntentEnqueueSystem enqueuer, IEntityIndex indexer) 
        : base(world)
    {
        _logger = logger;
        _enqueuer = enqueuer;
    ...
    private void ConsumeExitGameIntents()
    {
        ...
        while (processed < MaxPerTick && _enqueuer.ExitGameQueue.TryDequeue(out var intent))
        {
            ...
            var entity = _cmd.Create([Component<ExitGameIntent>.ComponentType]);
            _cmd.Set(entity, intent);
            processed++;
        }
    }
